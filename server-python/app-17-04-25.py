...
with open("docs/asp-onboarding.txt", "r", encoding="utf-8") as f:
        project_docs = f.read()


def serialize_message(msg):
    return {"role": msg.role, "parts": [str(part) for part in msg.parts]}
...
def chat():
    ...
    with open("docs/chat_history.json", "r", encoding="utf-8") as f:
       history_serialized = json.load(f)
    ...
    #   {"role": "user", "parts": [f"Here is the project documentation:\n{project_docs}"]},
    #   {"role": "model", "parts": ["Understood. I will use this documentation for future responses."]}
    # ])

    chat = model.start_chat(history=history_serialized)
    ...
    history_serialized = [serialize_message(m) for m in chat.history]
    with open("docs/chat_history.json", "w", encoding="utf-8") as f:
        json.dump(history_serialized, f)
